<template>
  <div class="course-container">
    ...
    <div class="header-nav">
      ...
      <a v-if="isAdmin" href="#" @click.prevent="goToAuditList">课程审核</a>
    </div>
  ...
  setup() {
    ...
    const fetchCourses = async () => {
      try {
        loading.value = true;
        error.value = null;
        const response = await axios.get('http://localhost:8848/course/get-all-course');
    ...
    const confirmUpdate = async () => {
      try {
        ...
        if (updateCoverFile.value) {
          ...
          const uploadResponse = await axios.post('http://localhost:8848/course/upload', formData, {
        ...
        const response = await axios.post('http://localhost:8848/course/update-course', updateData);
    ...
    const confirmDelete = async () => {
      try {
        for (const courseId of selectedCourses.value) {
          const response = await axios.get(`http://localhost:8848/course/delete-course?courseId=${courseId}`);
          if (!response.data || response.data.code !== 200) {
    ...
    const saveCourse = async () => {
      try {
        ...
        const response = await axios.post('http://localhost:8848/course/add-course', courseToSave);
